CREATE OR REPLACE PROCEDURE P3.CUST_CRT (
	...
	OUT err_msg varchar(255)
  	...
  	DECLARE EXIT HANDLER FOR SQLEXCEPTION
  		SET retcode = SQLCODE;
  	IF age < 0 THEN
  		SET err_msg = 'Invalid Input: Age cannot be negative';
  		RETURN -1;
  	END IF;
  	IF pin < 0 THEN
  		SET err_msg = 'Invalid Input: PIN cannot be negative';
  		RETURN -1;
  	END IF;
  	If pin < 999 THEN
  		SET err_msg = 'Invalid Input: PIN must be more than 4 digits';
  		RETURN -1;
  	END IF;
...
CREATE OR REPLACE PROCEDURE P3.ACCT_OPN (
	...
	DECLARE tempaccid INTEGER;
  	...
  	IF ini_balance < 0 THEN
  		SET err_msg = 'Invalid Input: Initial deposit cannot be negative';
  		RETURN -1;
  	END IF;
...
CREATE OR REPLACE PROCEDURE P3.ACCT_DEP (
	...
	OUT err_msg varchar(255)
  	...
  	DECLARE EXIT HANDLER FOR SQLEXCEPTION
  		SET retcode = SQLCODE;
  	IF dep_amt < 0 THEN
  		SET err_msg = 'Invalid Input: Deposit amount cannot be negative';
  		RETURN -1;
  	END IF;
...
CREATE OR REPLACE PROCEDURE P3.ACCT_WTH (
	...
	OUT err_msg varchar(255)
  	...
  	DECLARE EXIT HANDLER FOR SQLEXCEPTION
  		SET retcode = SQLCODE;
  	IF wth_amt < 0 THEN
  		SET err_msg = 'Invalid Input: Withdraw ammount cannot be negative';
  		RETURN -1;
  	END IF;
...
CREATE OR REPLACE PROCEDURE P3.ACCT_TRX (
	...
	OUT err_msg varchar(255)
  	...
  	DECLARE EXIT HANDLER FOR SQLEXCEPTION
   	...
   	IF trx_amt < 0 THEN
  		SET err_msg = 'Invalid Input: Transfer amount cannot be negative';
  		RETURN -1;
  	END IF;
	...
	OUT err_msg varchar(255)
  	...
  	DECLARE EXIT HANDLER FOR SQLEXCEPTION
   	...
   	IF savings_rate < 0 THEN
  		SET err_msg = 'Invalid Input: Interest rates cannot be negative';
  		RETURN -1;
  	END IF;
  	IF savings_rate > 1 THEN
  		SET err_msg = 'Invalid Input: Interest rates cannot be greater than 100%';
  		RETURN -1;
  	END IF;
